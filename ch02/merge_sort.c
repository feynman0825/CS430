...
void another_merge(int *A, int p, int q, int r) {
  ...
  // the left part to the target, if the left part exhaust it's elements, we do
  // nothing, because it's already sorted in the target
  if (rightHalfElementTraveled == rightHalfElementCount) {
    memcpy(target, leftStart + leftHalfElementTraveled,
